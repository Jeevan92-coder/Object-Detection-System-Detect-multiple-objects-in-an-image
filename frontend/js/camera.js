...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function updateDetList(dets) {
    if (dets.length === 0) {
      liveDets.innerHTML='<div style="font-size:12px;color:var(--txt3);font-family:var(--mono)">No objects in frame…</div>';
      return;
    }
  ...
  function setStatus(st) {
    ...
    lsStatus.style.color = st==="ACTIVE"?"var(--green)":st==="IDLE"?"var(--txt3)":"var(--red)";
  }
