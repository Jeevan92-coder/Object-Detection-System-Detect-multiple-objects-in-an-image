...
start "ODS Backend" cmd /k "title ODS Backend & chcp 65001 > nul & cd /d "%BACKEND_DIR%" & "%VENV_PYTHON%" -W ignore app.py"

timeout /t 3 /nobreak > nul

echo  [3/3] Frontend start ho raha hai...
start "ODS Frontend" cmd /k "title ODS Frontend & cd /d "%FRONTEND_DIR%" & python -m http.server 8080"
...
echo  Status   : Both services running
echo.
echo  Dono terminal ko band mat karo! (Don't close either terminal!)
echo.
echo  ==========================================
